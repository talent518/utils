...
typedef struct ftpbuf {
	...
	unsigned int    timeout_sec;	/* User configurable timeout (seconds) */
...
#endif
	int             disconnect; // if disconnected
	int             reconnect; // reconnect times
	...
	char            *pass;
	int             pass_len;
	void (*progress)(struct ftpbuf *ftp, databuf_t *data, size_t rcvd, size_t sent);
	size_t                     total;
	size_t                     rcvd;
	size_t                     sent;
} ftpbuf_t;
...
int       ftp_reconnect(ftpbuf_t *ftp);
 ...
 */
char* ftp_mkdir(ftpbuf_t *ftp, const char *dir, const size_t dir_len);
...
int		ftp_alloc(ftpbuf_t *ftp, const zend_long size, char **response);
